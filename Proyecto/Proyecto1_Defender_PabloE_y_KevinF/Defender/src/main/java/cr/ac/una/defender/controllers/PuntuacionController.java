...
public class PuntuacionController implements Initializable
{
    ...
    public void llenartodo()// se encarga de crear una tabla donde se muestran todos los jugadores de la base de datos 
    {
        try
        {
            ...
            TableColumn<UserDto , String> tbcNivel = new TableColumn<>("Nivel");
            tbcNivel.setPrefWidth(150);
            tbcNivel.setCellValueFactory(cd -> cd.getValue().lvl);

            TableColumn<UserDto , String> tbcDinero = new TableColumn<>("Dinero");
            tbcDinero.setPrefWidth(150);
            tbcDinero.setCellValueFactory(cd -> cd.getValue().dinero);

            TableColumn<UserDto , String> tbcGemas = new TableColumn<>("Esmeraldas");
            tbcGemas.setPrefWidth(150);
            tbcGemas.setCellValueFactory(cd -> cd.getValue().esmeralda);
