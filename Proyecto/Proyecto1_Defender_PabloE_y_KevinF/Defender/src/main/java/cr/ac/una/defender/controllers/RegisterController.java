...
public class RegisterController extends Controller implements Initializable
{
    ...
    private void click(ActionEvent event)
    {
        ...
        if(event.getSource() == Btn_register)
        {
            ...
            {
                new Mensaje().showModal(Alert.AlertType.WARNING , "Usuario " , Btn_back.getScene().getWindow() , "Por favor digite el usuario");

            }
            else if((Txf_pass.getText().isEmpty() || Txf_pass1.getText().isEmpty()))
            {
                new Mensaje().showModal(Alert.AlertType.INFORMATION , "Contraseñas " , Btn_back.getScene().getWindow() , "No digito alguna de las contraseñas");

            }
            else if(!Txf_pass.getText().equals(Txf_pass1.getText()))
            {

                new Mensaje().showModal(Alert.AlertType.WARNING , "Contraseñas " , Btn_back.getScene().getWindow() , "Por favor digite correctamente las contraseñas");
                Txf_pass.clear();
                Txf_pass1.clear();
            }
            else
            {
                ...
                new Mensaje().showModal(Alert.AlertType.INFORMATION , "Usuario " , Btn_back.getScene().getWindow() , "Usuario Registrado con exito");
                FlowController.getInstance().goVista("Login");
            }
        }
    }
